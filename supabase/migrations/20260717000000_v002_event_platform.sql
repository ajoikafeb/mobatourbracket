...
CREATE POLICY "Auth full access event categories" ON event_categories FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Auth full access events" ON events FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Auth full access registration forms" ON registration_forms FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Auth full access registration fields" ON registration_fields FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Auth full access registration responses" ON registration_responses FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Auth full access announcements" ON announcements FOR ALL USING (auth.role() = 'authenticated');
